...
int main()
{
	...
	//Esta macro evalúa la condición pasada como argumento. Si la condición es falsa, el programa se aborta y se genera un mensaje de error que incluye el nombre del archivo y el número de línea donde ocurrió el assert.
	assert(cin.good()); //cin.good() devuelve true si el estado del flujo de entrada está bien, es decir, no hay errores de entrada/salida, fin de archivo alcanzado, ni otros errores de estado.
	struct block
	{
		vector<int> elements;
		int parity;
	...
	for(auto& [elements, parity, num_even]: blocks)
	{
		num_even = 0;
		for(auto& x: elements)
			num_even += !(x & 1);//realiza una operación para contar números pares. 
	...
	for(int i = 0; i < m; ++i)
	{
		...
		if(nums[x] ^ y) //different parity
		{
			...
			blocks[block_pos[x].first].parity = blocks[block_pos[x].first].num_even == 0;
			parity_of_odd_blocks &= 1;
		}
